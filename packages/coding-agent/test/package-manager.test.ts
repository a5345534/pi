...
Content`,
		...
		it("should auto-discover root markdown skills from .pi-fork skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .pi-fork", async () => {
			const extDir = join(tempDir, ".pi-fork", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".pi-fork"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".pi-fork", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".pi-fork", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".pi-fork", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".pi-fork", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".pi-fork", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .pi-fork/agent skills", async () => {
		...
		it("should use the project .pi-fork dir as baseDir for project .pi-fork skills", async () => {
			const projectBaseDir = join(tempDir, ".pi-fork");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".pi-fork", "agent");
		...
		it("should dedupe user skill entries when ~/.pi-fork/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .pi-fork auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".pi-fork\n");

			const skillDir = join(tempDir, ".pi-fork", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.pi-fork\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".pi-fork", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .pi-fork settings base", () => {
			...
			const rel = relative(join(tempDir, ".pi-fork"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".pi-fork", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".pi-fork", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".pi-fork", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".pi-fork", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".pi-fork", "npm", "node_modules", "example");
